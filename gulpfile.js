...
gulp.task('connect', function(){
	connect.server({ 	//настраиваем конфигурацию сервера
		root: [path.outputDir], // корневая директория сайта
		port: 9999, // порт вывода этой херни
		livereload: true // инициализация работы LiveReload		
	});	
});
...
gulp.task('sprites:build', function() {
	var spriteData =
		gulp.src(path.src.sprites) // ds,выберем откуда брать изображения для объединения в спрайты
			.pipe(spritesmith({
				imgName: 'sprite.png', // имя спрайтового изображения
				cssName: 'sprite.styl', // имя стиля, где мы храним позиции изображений в спрайте
				imgPath: 'images/sprite.png', // путь, где лежит спрайт
				cssFormat: 'stylus', // формат,  в котром обрабатываем позиции
...
//Херачим динамичные изображения
